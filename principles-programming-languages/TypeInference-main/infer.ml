...
let rec gen (env: environment) (e: expr): aexpr * typeScheme * (typeScheme * typeScheme) list =
  ...
  | ID x -> 
      if List.mem_assoc x env then
        ...
        raise UndefinedVar
  | Fun (x, body) ->
      let alpha = gen_new_type () in
      let beta = gen_new_type () in
      let (ae, t, q) = gen ((x, alpha) :: env) body in
      (AFun(x, ae, TFun(alpha, beta)), TFun(alpha, beta), q @ [(t, beta)])
  | Not e ->
      let (ae, t, q) = gen env e in
      (ANot(ae, TBool), TBool, q @ [(t, TBool)])
  | Binop (op, e1, e2) ->
      let (ae1, t1, q1) = gen env e1 in
      let (ae2, t2, q2) = gen env e2 in
      let (ret_type, constraints) = match op with
        | Add | Sub | Mult | Div -> (TNum, [(t1, TNum); (t2, TNum)])
        | Concat -> (TStr, [(t1, TStr); (t2, TStr)])
        | Greater | Less | GreaterEqual | LessEqual | Equal | NotEqual -> (TBool, [(t1, t2)])
        | And | Or -> (TBool, [(t1, TBool); (t2, TBool)])
      in
      (ABinop(op, ae1, ae2, ret_type), ret_type, q1 @ q2 @ constraints)
  | If (e1, e2, e3) ->
      let (ae1, t1, q1) = gen env e1 in
      let (ae2, t2, q2) = gen env e2 in
  ...
  | FunctionCall (e1, e2) ->
      ...
      let (ae2, t2, q2) = gen env e2 in
      let alpha = gen_new_type () in
      (AFunctionCall(ae1, ae2, alpha), alpha, q1 @ q2 @ [(t1, TFun(t2, alpha))])
  | Let (x, is_rec, e1, e2) ->
      if is_rec then
        let alpha = gen_new_type () in
        let (ae1, t1, q1) = gen ((x, alpha) :: env) e1 in
        let (ae2, t2, q2) = gen ((x, t1) :: env) e2 in
        (ALet(x, is_rec, ae1, ae2, t2), t2, q1 @ [(alpha, t1)] @ q2)
      else
        let (ae1, t1, q1) = gen env e1 in
        let (ae2, t2, q2) = gen ((x, t1) :: env) e2 in
        (ALet(x, is_rec, ae1, ae2, t2), t2, q1 @ q2)
  | Cons (e1, e2) ->
      let (ae1, t1, q1) = gen env e1 in
      let (ae2, t2, q2) = gen env e2 in
      let ltype = TList t1 in
      (ACons(ae1, ae2, ltype), ltype, q1 @ q2 @ [(t2, ltype)])
...
let rec unify (constraints: (typeScheme * typeScheme) list) : substitutions =
  ...
  | (x, y) :: xs ->
    ...
    t1 @ (List.map (fun (k, v) -> (k, apply t1 v)) t2)
...
and unify_one (t1: typeScheme) (t2: typeScheme) : substitutions =
  ...
  | T(x), z | z, T(x) -> 
      if z = T(x) then []
      else if occurs x z then raise OccursCheckException
      else [(x, z)]
